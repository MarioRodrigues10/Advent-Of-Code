defmodule Day1_Part1_V2 do
  def sum_values_lines(input) do
    lines = String.split(input, ~r/\n/)
    values = lines |> Enum.map(&get_digits/1)
    Enum.sum(values)
  end
  ...
  defp get_digits(str) do
   ...
   {first_digit, second_digit} = {length > 0 && String.at(digits, 0) |> String.to_integer() || 0, length > 0 && String.at(digits, length - 1) |> String.to_integer() || 0}
...
result = Day1_Part1_V2.sum_values_lines(input)
IO.puts("Sum: #{result}")
