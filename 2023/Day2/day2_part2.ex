defmodule Day2_Part2 do
  ...
  defp parse_game(game) do
    [game_id | rounds] = String.split(game, ":")
    game_id = String.split(game_id, " ")
    game_id = String.to_integer(Enum.at(game_id, 1))
    rounds = Enum.at(rounds, 0)

    rounds
    |> String.split([";", ","])
    |> Enum.flat_map(&parse_round(&1, game_id))
  end

  defp parse_round(round, game_id) do
    String.split(round, ",")
    |> Enum.map(&parse_color(&1, game_id))
  end
  ...
  def sum_games(games) do
    ...
    red = Map.get(highest_counts, :red, 0)
    green = Map.get(highest_counts, :green, 0)
    blue = Map.get(highest_counts, :blue, 0)
