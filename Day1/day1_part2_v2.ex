defmodule Day1_Part2_V2 do
  ...
  defp extract_value(line) do
    words = line |> generate_substrings
    values = words |> Enum.map(&word_to_digit/1)
    first_value = Enum.find(values, fn x -> x != 10 end)
    second_value = Enum.find(Enum.reverse(values), fn x -> x != 10 end)

    case {first_value, second_value} do
      {nil, nil} -> 0
      {nil, _} -> second_value * 10 + second_value
      {_, nil} -> first_value * 10 + first_value
      {_, _} -> first_value * 10 + second_value
    end
  end
  ...
  def generate_substrings(line) do
    ...
    if String.match?(cleaned_line, ~r/[^[:space:]]/) do
      for start <- 0..(String.length(cleaned_line) - 1),
          stop <- start..(String.length(cleaned_line) - 1),
          into: [],
          do: String.slice(cleaned_line, start..stop)
    else
      []
    end
  end
...
result = Day1_Part2_V2.sum_values(input)
IO.puts("Sum: #{result}")
